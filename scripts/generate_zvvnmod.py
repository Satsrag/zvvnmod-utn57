...
@dataclass(frozen=True)
class Utn57ReverseRule:
    """One reviewed UTN #57 sequence → ZVVNMOD sequence row."""

    id: str
    sources: tuple[Utn57Target, ...]
    targets: tuple[CodeEntry, ...]


def read_utn57_reverse_mapping_csv(
    path: Path,
    forward_path: Path,
    model: Model,
    targets: tuple[Utn57Target, ...],
) -> tuple[Utn57ReverseRule, ...]:
    """读取并验证 reverse mapping CSV。 / Read and validate the reverse mapping CSV."""
    ...
    try:
        ...
        raise ValueError(f"UTN57 reverse mapping CSV: {error}") from error
    if metadata["schema"] != "utn57-zvvnmod-runtime-map-v1":
        raise ValueError("UTN57 reverse mapping CSV metadata differs from schema")
    forward_digest = hashlib.sha256(forward_path.read_bytes()).hexdigest()
    if metadata["baseline"] != f"sha256:{forward_digest}":
        raise ValueError(
            "UTN57 reverse mapping baseline does not match the forward map; "
            "re-run scripts/derive_utn57_zvvnmod_map.py"
        )

    rules: list[Utn57ReverseRule] = []
    row_ids: set[str] = set()
    seen_sources: set[tuple[str, ...]] = set()
    for index, row in enumerate(rows):
        row_id = row["id"]
        if re.fullmatch(r"[A-Za-z0-9:_-]+", row_id) is None or row_id in row_ids:
            raise ValueError(f"reverse mapping {index}: invalid or duplicate ID {row_id!r}")
        row_ids.add(row_id)
        if row["sources"] == "":
            raise ValueError(f"reverse mapping {row_id}: source sequence must be non-empty")
        source_ids = tuple(row["sources"].split(" "))
        target_ids = tuple(row["targets"].split(" ")) if row["targets"] else ()
        if any(
            not item or any(character.isspace() for character in item)
            for item in source_ids + target_ids
        ):
            raise ValueError(f"reverse mapping {row_id}: sequences must use single spaces")
        if source_ids in seen_sources:
            raise ValueError(f"reverse mapping {row_id}: duplicate source sequence")
        seen_sources.add(source_ids)
        try:
            source_entries = tuple(targets_by_id[item] for item in source_ids)
        except KeyError as error:
            raise ValueError(
                f"reverse mapping {row_id}: unknown source {error.args[0]!r}"
            ) from error
        try:
            target_entries = tuple(code_by_name[item] for item in target_ids)
        except KeyError as error:
            raise ValueError(
                f"reverse mapping {row_id}: unknown target {error.args[0]!r}"
            ) from error
        rules.append(Utn57ReverseRule(row_id, source_entries, target_entries))

    if not rules:
        raise ValueError("UTN57 reverse mapping CSV must contain at least one relation")
    spelled = {rule.id for rule in rules if rule.targets}
    if not spelled:
        raise ValueError("UTN57 reverse mapping CSV must spell at least one unit")
    # Every inventory unit needs a row, so conversion never falls off the table.
    covered = {rule.sources[0].id for rule in rules if len(rule.sources) == 1}
    missing = sorted(target.id for target in targets if target.id not in covered)
    if missing:
        raise ValueError(f"reverse mapping is missing rows for {', '.join(missing)}")
    return tuple(rules)


def render_utn57_reverse_mapping_rust(
    rules: tuple[Utn57ReverseRule, ...], mapping_source: str
) -> str:
    """渲染 reverse mapping Rust 表。 / Render the reverse mapping Rust table."""
    ...
    for rule in rules:
        source_names = [target.const_name for target in rule.sources]
        target_names = [entry.const_name for entry in rule.targets]
        lines.append(f"    // {rule.id}")
        lines.append("    Utn57ToZvvnmodMapping {")
        lines.append(f'        id: "{rule.id}",')
        for field, names in (("sources", source_names), ("targets", target_names)):
            inline = f"        {field}: &[{', '.join(names)}],"
            if len(inline) <= 88:
...
def generate_utn57_reverse_mapping(
    names_path: Path,
    targets_path: Path,
    forward_path: Path,
    reverse_path: Path,
    ...
    """生成 reverse mapping Rust 表。 / Generate the reverse mapping Rust table."""

    model = build_model(read_csv(names_path))
    targets = read_utn57_targets_csv(targets_path)
    rules = read_utn57_reverse_mapping_csv(reverse_path, forward_path, model, targets)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(
        render_utn57_reverse_mapping_rust(rules, reverse_path.name), encoding="utf-8"
    )
    return rules
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=root / "data" / "utn57-zvvnmod-map.csv",
    )
    parser.add_argument(
        "--reverse-mapping-output",
        type=Path,
        default=root / "src" / "generated" / "utn57_reverse_mapping.rs",
    )
    ...
    reverse = generate_utn57_reverse_mapping(
        args.input,
        args.targets_input,
        args.mapping_input,
        args.reverse_mapping_input,
        args.reverse_mapping_output,
    )
    print(
        f"generated {len(model.codes)} codes, "
        f"{len(model.code_decompositions)} code decompositions, "
        f"{len(rules)} Ir_fina replacements, "
        f"{len(mapping.targets)} UTN57 targets, {len(mapping.rules)} mapping rows, "
        f"and {len(reverse)} reverse mapping rows -> "
        f"{args.codes_output}, {args.decomposition_map_output}, {args.ir_fina_output}, "
        f"{args.mapping_output}, {args.reverse_mapping_output}"
    )
