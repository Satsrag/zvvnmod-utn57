...
def read_csv(path: Path) -> list[InputRow]:
    """读取并校验权威 CSV。 / Read and validate the authoritative CSV."""
...
def render_codes_rust(model: Model, source_name: str) -> str:
    """生成纯英文注解的 Rust 编码定义。 / Render Rust code definitions with English-only docs."""
...
def render_shape_map_rust(model: Model, source_name: str) -> str:
    """生成纯英文注解的 Rust shape Map。 / Render the Rust shape map with English-only docs."""
